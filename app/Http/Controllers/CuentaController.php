...
class CuentaController extends Controller
{
    ...
    // {
    //     // 1. Cargar relaciones esenciales
    //     $cuenta->load([
    //         'checkin.cliente.usuario',
    //         'checkin.habitacionEvento.tipoHabitacion',
    //         'transaccions.servicio',
    ...
    //     // Calcular noches/días de estancia (mínimo 1 día si ya entró)
    //     // Usamos diffInDays para contar las noches completas.
    //     $noches = $fechaSalida->diffInDays($fechaEntrada);
    //     // Si la estancia es menos de un día o justo un día, aseguramos mínimo 1 noche cobrada.
    //     $noches = max(1, $noches); 

    //     $precioNoche = $checkin->habitacionEvento->tipoHabitacion->precio;
    //     $costoAlojamiento = $precioNoche * $noches;
        
    //     // 2. Preparar el Desglose de Transacciones
    //     $desgloseItems = collect();

    //     // A. Cargo Base de Alojamiento (el primer cargo)
    //     $desgloseItems->push([
    //         'descripcion' => 'Alojamiento (' . $checkin->habitacionEvento->codigo . ' - ' . $checkin->habitacionEvento->tipoHabitacion->nombre . ')',
    //         'tipo' => $checkin->habitacionEvento->tipoHabitacion->tipo === 'habitacion' ? 'Habitación' : 'Evento',
    //         'precio_unitario' => $precioNoche,
    //         'cantidad' => $noches,
    //         'subtotal' => $costoAlojamiento,
    //         'estado' => 'CONFIRMADO',
    //         'fecha' => $fechaEntrada->format('d/m/Y'),
    //     ]);
    ...
    //             'cantidad' => $t->cantidad,
    //             'subtotal' => $t->subtotal,
    //             'estado' => strtoupper($t->estado), // Usamos el estado de la transacción (CANCELADA, PENDIENTE)
    //             'fecha' => Carbon::parse($t->created_at)->format('d/m/Y H:i'),
    //         ]);
    //     }
        
    //     // C. Calcular Totales Finales (para referencia, aunque ya están en $cuenta)
    //     $totalConsumosActivos = $desgloseItems->where('estado', '!=', 'CANCELADA')->sum('subtotal');
        
    //     // 3. Preparar los datos generales del reporte
    //     $datosReporte = [
    //         'titulo' => $cuenta->factura ? 'FACTURA OFICIAL' : 'REPORTE DE CUENTA',
    //         'cuenta' => [
    //             'id' => $cuenta->id,
    //             'monto_total' => $cuenta->monto_total, // Monto total de la DB
    //             'monto_pagado' => $cuenta->monto_pagado,
    //             'saldo' => $cuenta->saldo,
    //             'estado' => $cuenta->estado,
    ...
    //         ],
    //         'estadia' => [
    //             'habitacion' => $checkin->habitacionEvento->codigo . ' - ' . $checkin->habitacionEvento->nombre,
    //             'fecha_entrada' => $fechaEntrada->format('d/m/Y H:i'),
    //             'fecha_salida' => $checkin->fecha_salida ? $fechaSalida->format('d/m/Y H:i') : 'EN CURSO',
    //             'noches' => $noches,
    //         ],
    //         'factura' => $cuenta->factura,
    //         'desglose' => $desgloseItems,
    //         'total_confirmado_calculado' => $totalConsumosActivos, // Total sin ítems cancelados
    //     ];

    //     // 4. Generar y Devolver el PDF
    //     $pdf = Pdf::loadView('pdf.reporte_cuenta', $datosReporte);
